...
show_menu() {
  ...
  echo -e "  ${BOLD}8)${NC}  🚀  Run All (Local)    ${DIM}— Launch all detected runners${NC}"
  echo -e "  ${BOLD}9)${NC}  🔄  Update ci-tools    ${DIM}— Update submodule to latest main${NC}"
  echo -e "  ${BOLD}10)${NC} ➕  Create Outbox      ${DIM}— Scaffold a new outbox project${NC}"
  echo -e "  ${BOLD}11)${NC} 🔄  Safe Rebase        ${DIM}— Stash, rebase & pop changes${NC}"
...
while true; do
  ...
  case "${choice}" in
    ...
    6)
      ...
      OUTBOXES=$(get_outboxes)
      for outbox in $OUTBOXES; do
          echo -e "${CYAN}▸ Checking ${BOLD}${outbox}${NC}..."
          (
              cd "$outbox"
              UPDATED=false
