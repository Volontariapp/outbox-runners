...
import { StorageJobsOutboxRunner, StorageEventQueueRunner } from './runners/index.js';
...
async function bootstrap() {
  ...
  const logger = new Logger({
    context: 'OUTBOX-STORAGE',
    format: config.logger.format,
  });

  logger.info('Outbox runner for storage starting...');
  ...
  const jobsRunner = new StorageJobsOutboxRunner(
    jobsRepository,
    config.outbox,
    logger,
    jobsPusher,
  ...
  const eventsRunner = new StorageEventQueueRunner(
    eventsRepository,
    config.outbox,
    logger,
    eventsPusher,
